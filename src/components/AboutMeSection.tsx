        ...
        <div className="max-w-9xl mx-auto">
          <div className="flex lg:flex-row flex-col-reverse justify-between items-center gap-8">
            ...
            <div className="flex-1">
              <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-[3.157rem] font-normal leading-tight text-center lg:text-left text-gray-100 dark:text-gray-950 transition-colors duration-200">
              Hey, I'm Welinton, and I <em>design experiences</em> that <em>capture attention</em> and inspire curiosity.
              </h2>
              <p className="mt-8 max-w-6xl text-lg text-center lg:text-left text-gray-300 dark:text-gray-700 transition-colors duration-200">
...
function Feature({ icon, label }: FeatureProps) {
  return (
    <div className="flex items-center gap-4 theme__bg-soap rounded-full px-4 py-2 transition-colors duration-200">
      <div className="flex justify-center items-center isolate-color dark:text-gray-100">{icon}</div>
      <p className="paragraph-no-margin isolate-color dark:text-gray-100">{label}</p>
    </div>
  );
}
